...
class CapacityWebSolutions_ImportProduct_Model_Convert_Adapter_Product extends Mage_Catalog_Model_Convert_Adapter_Product
{
	...
	protected function addTogtherSell($product, $userData, $ordinal = 0)
    {   
		...
		$result = $read->query("SELECT * FROM catalog_product_link_type WHERE 
            code='".ManaPro_ProductPlusProduct_Resource_Setup::LINK_TYPE."'");
        $tempresult = $result->fetch(PDO::FETCH_ASSOC);  
		$togtherid=$tempresult['link_type_id'];


		$result = $read->query("SELECT * FROM catalog_product_link_attribute  WHERE 
            link_type_id=".$togtherid." and product_link_attribute_code='position'");
        $tempresult = $result->fetch(PDO::FETCH_ASSOC);  
		$togthertempid=$tempresult['product_link_attribute_id'];
        ...
        foreach ( $this->userCSVDataAsArray( $userData ) as $oneSku ) {
    		if ( ( $a_sku = ( int )$product->getIdBySku( $oneSku ) ) > 0 ) {
                ...
                $write->query("INSERT into $linkTable SET
                    ...
                    linked_product_id=".$a_sku.", 
                    link_type_id=".$togtherid);
                $result = $read->query("SELECT * FROM $linkTable WHERE
                    product_id=".$productId." and
                    linked_product_id=".$a_sku." and
                    link_type_id=".$togtherid);
                $link = $result->fetch(PDO::FETCH_ASSOC);   

                $write->query("INSERT into catalog_product_link_attribute_decimal SET
                    product_link_attribute_id = ".$togthertempid.",
    ...
    public function saveRow(array $importData)
    {
		...
		if (isset($importData['mproductplusproduct'])) {

			$this->addTogtherSell($product,$importData['mproductplusproduct']);
            } 
