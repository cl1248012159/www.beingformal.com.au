...
class Iifire_Tags_Adminhtml_Itag_IndexController extends Mage_Adminhtml_Controller_Action  
{
    ...
    public function saveAction()
    {
    	...
    	$qImportant = trim($this->getRequest()->getParam('q_important'));
    	$storeId = (int)$this->getRequest()->getParam('store_id');
    	$categoryId = (int)$this->getRequest()->getParam('category_id');
    	$tagsType = (int)$this->getRequest()->getParam('tags_type');
    	if ($q) {
    ...
    public function resetDescriptionAction()
    {
		...
		try {
			$write->query("update {$write->getTableName('iifire_tags')} set tags_description=NULL;");
    ...
    public function resetTagsProductsAction()
    {
    	$write = Mage::getSingleton('core/resource')->getConnection('core_write');
		try {
			$write->query("update {$write->getTableName('iifire_tags')} set tags_products=NULL;");
    ...
    public function resetImportantTagsAction()
    {
    	$write = Mage::getSingleton('core/resource')->getConnection('core_write');
		try {
			$write->query("update {$write->getTableName('iifire_tags')} set random_tags=NULL;");
