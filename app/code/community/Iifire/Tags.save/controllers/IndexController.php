...
class Iifire_Tags_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function viewAction()
    {
		...
		if (!$query->getId()) {
			...
			$queryModel->setQueryText($query->getQueryText())
				->setIsActive(1)
				->setDisplayInTerms(1)
    	...
    	if ($query->getQueryText()) {
			...
			if ($q) {
				...
				$title = !$p ? str_replace('{page}', '', $title) : str_replace('{page}', ' - '.$helper->__('Page ').$p, $title);
				$head->setTitle(str_replace("{}", $q, $title));
				$head->setKeywords(str_replace("{}",$q,$helper->getViewKeywords()));
				$head->setDescription(str_replace("{}",$q,$helper->getViewDescription()));
			}
			//echo 'yang';die();
	    	$this->renderLayout();
    	} else {
