 ...
 * @version 1.0.0
 */
class Iifire_Tags_Block_Adminhtml_Tags_Category extends Mage_Adminhtml_Block_Template
{
    public function __construct()
    {
        parent::__construct();
        $this->setTemplate('iifire/tags/category.phtml');
    }
    ...
    protected function _prepareLayout()
    {
        $this->setChild('new_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                    'label'     => Mage::helper('itags')->__('Add Category'),
                    'onclick'   => "window.location.href = '" . $this->getUrl('*/*/add') . "'",
                    'class'     => 'add'
                ))
        );
    ...
    {
        return Mage::helper('itags')->__('Tags Category');
    }
}
