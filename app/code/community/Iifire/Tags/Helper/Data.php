...
class Iifire_Tags_Helper_Data extends Mage_Core_Helper_Abstract
{
	...
	public function createProductItem($loc, $changefreq,$priority='',$c,$baseurl)
    {	$lastmod=date("Y-m-d");
    	$priority = !$priority ? $this->getSitemapPriority() : $priority;
    	$item = "<url>\n";
		$item .= "<loc>" . $loc . "</loc>\n";
		...
		$item .= "<priority>" . $priority. "</priority>\n";
		if($c->getGallery()){
			foreach($c->getGallery() as $g){
				$item .= "<image:image>\n";
				$item .= "<image:loc>".$baseurl."media/catalog/product".$g['file']."</image:loc>\n";
				$imgtitle=str_replace("&"," ",$g['label']);
				$item .= "<image:title>".$imgtitle."</image:title>\n";
				$item .= "</image:image>\n";
			}
		}
		$item .= "</url>\n";   
