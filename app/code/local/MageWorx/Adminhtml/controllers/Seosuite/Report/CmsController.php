    ...
    public function runGenerateAction() {                        
        ...
        switch ($action) {
            ...
            case 'calculation': 
                ...
                $result = array();        
                if ($current<$total) {
                    if (count($stories)>=$total) $storeId = 0; else $storeId = $stories[$current];                    
                    ...
                    if ($current<=$limit) {
                        ...
                        $result['text'] = $this->__('Total %1$s, processed %2$s records (%3$s%%)...', $total-1, $current-1, round(($current-1)*100/($total-1), 2));
                    }    
