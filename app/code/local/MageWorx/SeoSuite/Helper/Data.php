...
class MageWorx_SeoSuite_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getLayerFilterUrl($params) {
        ...
        if (!empty($layerParams)) {
            ...
            $layer = implode('_', $layerParams);			//$layer = implode('_', $layerParams);
        }
        ...
        //return $url . '/l/' . $layer . '/kfc/'.$suffix . 'k/fc2/'.(isset($urlParts[1]) ? '?' . $urlParts[1] : '');		
		//add by xinran		
		$rooturl = Mage::getBaseUrl('web'); 		
		$c_tmp_url = str_replace($rooturl,'',$url);		
		$c_tmp_category = str_replace('.html','',$c_tmp_url);
		$c_urlsp = count($layerParams) > 2 ? 'ns':'narrow';	

		$rurl = $rooturl.$c_urlsp.'/'.$layer.'_'.$c_tmp_category; 
		$exturl = '';
		if($queryParams){
			$extarr = array('order','dir');
			foreach($queryParams as $k=>$v){
				if(!in_array($k,$extarr)) continue;
				$exturl .= $k.'='.$v.'&';
			}
			$exturl = trim($exturl,'&');
		}
		if($exturl) $exturl = '?'.$exturl;
		return $rurl.$exturl;
    }
