                    ...
                    case 'price':
                        if ($product->getTypeId() == 'bundle') {
                         ...
                         //   echo "$_minimalPriceTax, $_maximalPriceTax"; exit;
                           $value = $_minimalPriceTax."-".$_maximalPriceTax;
                        ...
                        elseif ($product->getTypeId() == 'grouped') {
                            if($product->getMinimalPrice()) {
                                    ...
                                    $value = $price;
                        //            $value = Mage::helper('core')->__('Starting at').' '.Mage::helper('core')->currency($price,true,FALSE);
                                }
                        }
                        else {
                            if ($product->getPrice() > 0){
                                $price = Mage::helper('tax')->getPrice($product, $product->getPrice(), $includingTax);
                                $value = Mage::app()->getStore($product->getStore()->getId())->convertPrice($price, false, false);
                                $value = Mage::helper('core')->currency($value,true,FALSE);
                                if($product->getMinimalPrice() && $product->getMinimalPrice()<>$product->getPrice()) {
                                    $price = Mage::helper('tax')->getPrice($product, $product->getMinimalPrice(), $includingTax);
                                    $value = Mage::app()->getStore($product->getStore()->getId())->convertPrice($price, false, false);
                         //           $value = Mage::helper('core')->__('Starting at').' '.Mage::helper('core')->currency($value,true,FALSE);
                                }
                            }
                        }
                        break;
