...
class MageWorx_XSitemap_Model_Sitemap extends Mage_Core_Model_Abstract {
    ...
    public function generateXml($entity=false) {
        ...
        if (!$entity || $entity=='tag') {
            ...
            if ($productTags) {
                ...
                $tags = Mage::getModel('tag/tag')->getPopularCollection()
                                ->joinFields($this->getStoreId())
                                ->load();
                foreach ($tags as $item) {
                    $tagUrl = str_replace($mageUrl, $baseUrl, $item->getTaggedProductsUrl());
                    $tagUrl = str_replace($baseUrl."index.php/", $baseUrl, $tagUrl);
                    $xml = sprintf('<url><loc>%s</loc><lastmod>%s</lastmod><changefreq>%s</changefreq><priority>%.1f</priority></url>',
        ...
        if ((!$entity || $entity=='blog') && Mage::getStoreConfig('blog/blog/enabled')) {
			...
			//	echo $dDate; exit;
                $xml = sprintf('<url><loc>%s</loc><lastmod>%s</lastmod><changefreq>%s</changefreq><priority>%.1f</priority></url>',
                                htmlspecialchars($baseUrl . $defaultRote ."/". $item->getUrl()),
                                $dDate,
                                $changefreq,
                                $priority
                );
            ...
            foreach (array('post', 'page') as $type) {
                ...
                if (count($items) > 0) { 
                    foreach ($items as $item) {
                        $xml = sprintf('<url><loc>%s</loc><lastmod>%s</lastmod><changefreq>%s</changefreq><priority>%.1f</priority></url>',
                            htmlspecialchars($item->getPermalink()),
